...
/**
 * payments using Hipay Comprafacil Payshop:
 * @author Diogo Ferreira
 * @version $Id: hipayps.php 1111 2017-03-06 10:52:22Z $
 * @package VirtueMart
 * @subpackage payment
...
class plgVmPaymentHipayps extends vmPSPlugin {
	...
	public function getVmPluginCreateTableSQL () {
		...
		return $this->createTableSQL ('Payment Hipay Comprafacil Payshop Table');
	}
	...
	function plgVmConfirmedOrder ($cart, $order) {
		...
		$dbValues['entity'] = $account["account_entity"];
    ...
    $origin =$hipay_base_url.'?option=com_virtuemart&view=pluginresponse&task=pluginnotification&tmpl=hipayps&id=' . $dbValues['order_number'];
		...
		try
		{
			...
			$res = $client->getReferencePS($parameters);

			if ($res->getReferencePSResult)
			{
				...
				$this->mb_value = $value;
				$this->mb_val = $paymentDeadline;

				$dbValues['reference'] = $reference;
				$dbValues['entity'] = $entity;
				$dbValues['validity'] = $paymentDeadline;
				...
				$html = '<table cellpadding="6" cellspacing="2" style="width: 350px; height: 55px; margin: 10px 0 2px 0;border: 1px solid #ddd"><tr>
						<td style="background-color: #ccc;color:#313131;text-align:center;padding:1px 3px;" colspan="3">'.JText::_( 'VMPAYMENT_HIPAYPS_PAYMENT_DESCRIPTION' ) .'</td>
					</tr>
					<tr>
						<td rowspan="3" style="width:80px;padding: 0px 5px 0px 5px;vertical-align: middle;"><img src="'.JURI::root(true).'/images/stories/virtuemart/payment/payshop_vertical.jpg" style="margin-bottom: 0px; margin-right: 0px;width:60px;"></td>
						<td align="right" style="padding:1px 3px;">'.JText::_( 'VMPAYMENT_HIPAYPS_REFERENCE' ).'   </td>
						<td style="font-weight:bold;padding:1px 3px;">'.$reference.'</td>
					</tr>
					<tr>
						<td align="right" style="padding:1px 3px;">'.JText::_( 'VMPAYMENT_HIPAYPS_AMOUNT' ).'   </td>
						<td style="font-weight:bold;padding:1px 3px;">'.$amount.' &euro;</td>
					</tr>
					<tr>
						<td align="right" style="padding:1px 3px;">'.JText::_( 'VMPAYMENT_HIPAYPS_DUEDATE' ).'   </td>
						<td style="font-weight:bold;padding:1px 3px;">'.$paymentDeadline.'</td>
					</tr>'. "\n";
				$html .= '</table>' . "\n";
	...
	function plgVmOnShowOrderBEPayment ($virtuemart_order_id, $virtuemart_payment_id) {
		...
		$db = JFactory::getDBO();
		$q = "SELECT reference,entity,validity,sandbox FROM " . $db->getPrefix() . "virtuemart_payment_plg_hipayps WHERE virtuemart_order_id = ".$virtuemart_order_id." LIMIT 1";
		$db->setQuery($q);
		$payment_details = $db->loadObjectList();

		$html = '<table class="adminlist">' . "\n";
		$html .= $this->getHtmlHeaderBE ();
		$html .= $this->getHtmlRowBE ('HIPAYPS_TITLE_DESC', $paymentTable->payment_name);
		$html .= $this->getHtmlRowBE ('HIPAYPS_AMOUNT', number_format($paymentTable->payment_order_total,2));
		$html .= $this->getHtmlRowBE ('HIPAYPS_REFERENCE', $payment_details[0]->reference);
		$html .= $this->getHtmlRowBE ('HIPAYPS_DUEDATE', $payment_details[0]->validity);
		if ($payment_details[0]->sandbox == 1) $html .= $this->getHtmlRowBE ('HIPAYPS_SANDBOX', '');
		$html .= '</table>' . "\n";
		return $html;
	}
	...
	public function plgVmOnPaymentNotification() {
		...
		if (isset($_GET["id"])) {
			...
			$reference = str_replace("+", " ", $reference);
				...
				$db = JFactory::getDBO();
				$q = "SELECT virtuemart_order_id,reference,virtuemart_paymentmethod_id FROM " . $db->getPrefix() . "virtuemart_payment_plg_hipayps WHERE order_number = '".$idformerchant."' LIMIT 1";
				$db->setQuery($q);
